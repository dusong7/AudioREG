...
asseg_t *profile_to_seg(spfbuf_t *e, bigauss_t *bg, float frate)
{
  ...
  asseg_t *add_seg(asseg_t **, float, float, int,asseg_t **);
  ...
  for (i = 0; i < e->n; i++) {
	...
	if (ofmt & SPEECH && et2 != ASEG_NULL_TIME) {
	  ...
	  ns2++;
	  d2 += (et2 - st2);
	  if (add_seg(&seg, st + st2, st + et2, SPEECH,&last) == NULL) {
	    seg_list_free(&seg);
	    return(NULL);
	  }
	}
	...
	if (ofmt & SILENCE) {
	  ns1++;
	  d1 += (et1 - st1);
	  if (add_seg(&seg, st + st1, st + et1, SILENCE,&last) == NULL) {
	    seg_list_free(&seg);
	    return(NULL);
	  }
	}
 ...
 * the new segment.
 *xiongchenhui修改，增加了参数last，取消静态变量
 */
asseg_t *add_seg(asseg_t **seg, float st, float et, int label,asseg_t **last)
{
